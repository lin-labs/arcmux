// arcmux-test is the substrate scenariotest harness for arcmux. It is
// the fast, free, behavioral counterpart to scripts/validate.sh: where
// validate.sh proves the binaries COMPILE and the wire edges respond,
// this binary proves the substrate actually DOES the right thing when
// a human-style sequence of commands is issued via arcmux-cli.
//
// (No real agent runs here. For the agent-behavioral end-to-end
// harness that spawns real `claude -p` invocations, see cmd/arcmux-e2e.)
//
...
// Usage:
//
//	bin/arcmux-test                                # all scenarios
//	bin/arcmux-test --scenario bootstrap            # one scenario by name
//	bin/arcmux-test --scenario bootstrap,pulse-wake # subset
//	bin/arcmux-test --list                          # print scenario names
//	bin/arcmux-test --keep                          # keep temp dirs after pass
//	bin/arcmux-test --report-dir <path>             # override report dir
//	bin/arcmux-test --bin <path> --call <path>      # override binary paths
//
...
import (
	...
	"github.com/lin-labs/arcmux/internal/scenariotest/scenarios"
)

func main() {
	if err := run(os.Args[1:]); err != nil {
		fmt.Fprintf(os.Stderr, "arcmux-test: %v\n", err)
		os.Exit(1)
	}
}

func run(args []string) error {
	fs := flag.NewFlagSet("arcmux-test", flag.ContinueOnError)
...
// substrate-scenariotest evidence.
