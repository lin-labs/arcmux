...
//   - a per-run temp dir under $TMPDIR/arcmux-test-<ts>-<scenario>/
//   - a data_root and vault_root under that temp dir
//   - a unique project slug ("scenariotest-<ts>-<scenario>") and cmux workspace
//     prefix ("scenariotest-<ts>-<scenario>-") so any leftover cmux workspaces
//     are trivially identifiable and reapable
...
type Env struct {
	...
	TracePath       string
	WorkspacePrefix string // "scenariotest-<ts>-<scenario>-"
	ProjectSlug     string // "scenariotest-<ts>-<scenario>"
	ArcmuxBin       string
	CallBin         string
	BaseEnv         []string
...
func NewEnv(scenario, arcmuxBin, callBin string, baseEnv []string) (*Env, error) {
	...
	tempBase := os.TempDir()
	tempRoot := filepath.Join(tempBase, "arcmux-test-"+tag)
	...
	env.tracef("=== scenariotest scenario=%s temp=%s ===", scenario, tempRoot)
...
func (e *Env) StartDaemon(ctx context.Context, ready time.Duration) error {
	...
	cmd.Env = append(e.envBase(), "ARCMUX_E2E=1") // legacy marker; scenariotest sets the same flag
	cmd.Stdout = logFile
	cmd.Stderr = logFile
