...
type Runner struct {
	Scenarios       []Scenario
	ScenariosRoot   string // testdata/eval-scenarios/
	ReportDir       string
	Verbose         bool
...
func (r *Runner) Run(ctx context.Context, stdout io.Writer) error {
	...
	reportPath := filepath.Join(r.ReportDir, "eval-"+stamp+".json")

	fmt.Fprintf(stdout, "arcmux eval — %s PT\n", stamp)
	...
	fmt.Fprintf(stdout, "\noverall: %s\nreport:  %s\n", overall, reportPath)
	if overall != "pass" {
		return fmt.Errorf("eval: one or more scenarios failed (see %s)", reportPath)
	}
	return nil
}
